...
It combines live market data, statistical analysis (correlations, spreads), actionable signals (arb alerts, trade suggestions), portfolio tracking with live exchange sync, algorithmic backtesting, trade replay with projections, and optional **secure multi-exchange trading** (Coinbase CDP + Kraken) via Supabase.
...
- **Strategy engine is pure TypeScript**: [src/lib/strategyEngine.ts](./src/lib/strategyEngine.ts) imports nothing from React. It is the single source of truth for backtest logic. Keep it that way.
...
- Backtest or strategy changes: update the pure engine first, then the StrategyDashboard UI that consumes `BacktestResult`.
...
- New backtest strategies or richer tick generation in the strategy engine
...
- **AGENTS.md**: Full technical spec — project structure, every component/hook/store/lib responsibility, CSS variable glossary, state patterns, security model, exact deployment steps. Read it when you need precision.
